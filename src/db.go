...
func create_tables() {
	...
	execSQL("create table if not exists problem_topic (id integer primary key, problem_id integer, topic_id integer)")
...
func init_database(db_name string) {
	...
	AddProblemTopicSQL = prepare("insert into problem_topic (problem_id, topic_id) values (?, ?)")
